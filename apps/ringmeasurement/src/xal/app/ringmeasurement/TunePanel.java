...
//		yDiffPlot = new double[allBPMs.size()];
	...
	private void tuneByFit() {
		...
		*/	
		 ...
		 try{
		  FileReader fr = new FileReader("/home/weiyy/Share/CSNS/tbtdatax.txt");
		  BufferedReader br = new BufferedReader(fr);
		  Scanner sc = new Scanner(br).useDelimiter("[ ]*,+[ ]*|\\s*;+\\s*|\\s+");
		
		 FileReader fs = new FileReader("/home/weiyy/Share/CSNS/tbtdatay.txt");
		 BufferedReader bs = new BufferedReader(fs);
		 Scanner sd = new Scanner(bs).useDelimiter("[ ]*,+[ ]*|\\s*;+\\s*|\\s+");
			  for (int i = 0; i < 1024; i++) {
				  for (int j = 0; j<32; j++){
					  datax[j][i]= sc.nextDouble();	
					  datay[j][i]= sd.nextDouble();
				  }
			  }
		...
		for (int i = 0; i < allBPMs.size(); i++) {
				...
				// for PV Logger data
				if (!isOnline) {
					System.out.println(bpmMap.get(theBPM.getId()));
				...
				if (!badBPMs.contains(new Integer(i))) {
					...
					try {
						...
						System.out.println("xtune "+i+"="+xTune[i]);
						
					//	if (tuneMeasurement[i].getXPhase() > -360.                     //weiyy
					//			&& tuneMeasurement[i].getXPhase() < 0.) {
							
							if (tuneMeasurement[i].getXPhase() < 0.) {
							xPhase[i] = tuneMeasurement[i].getXPhase()
									- Math.floor(tuneMeasurement[i].getXPhase()
											/ Math.PI / 2.) * Math.PI * 2.;
						} else if (tuneMeasurement[i].getXPhase() >= 0.) {
			...
			//			if (tuneMeasurement[i].getYPhase() > -360.                        //weiyy
			//					&& tuneMeasurement[i].getYPhase() < 0.) {
						...
						if (xPhase[i] != 0. && yPhase[i] != 0. && xTune[i] > 0.6              
								&& yTune[i] > 0.6 && xTune[i] < 1&& yTune[i] < 1) {         //weiyy
							xSum += xTune[i];
							ySum += yTune[i];
			...
			/*	xModelPhase[i] = state.getBetatronPhase().getx()
				...
				//System.out.println("xMeasurePhase=" + goodBPMdata.get(goodBPMs.get(i))[0].doubleValue() );
				System.out.println(yPhaseDiff[i]);
				//System.out.println("  xDiffplot="+xDiffPlot[i] );
...
//			yPhDiffPlotPane.setDataArray(goodPosArry, yModelPhase);
	...
	//
	//	HashMap<String, double[][]> bpmMap = null;               by weiyy
		
		HashMap<String, double[][]> bpmMap = new HashMap<String, double[][]>();
		quadTableModel.setAppMode(isOnline);
		if (!isOnline) {
	/*		bpmPVLogId = myDoc.bpmPVLogId;                      by weiyy
			defPVLogId = myDoc.defPVLogId;
			System.out.println("bpmPVLogId" +bpmPVLogId); 
			RingBPMTBTPVLog pvLog = new RingBPMTBTPVLog(bpmPVLogId);
			bpmMap = pvLog.getBPMMap();
	*/	
			 double[][] datax = new double[32][1024];
		...
		for (int i = 0; i < allBPMs.size(); i++) {
			...
			Thread thread = new Thread(tuneMeasurement[i]);
				...
				try {
					...
					if (xTune[i] > 0.5 && xTune[i] < 1.0) {      //weiyy
						xSum += xTune[i];
						xList.add(new Double(xTune[i]));
						xCounter++;						
					}
					
					if (yTune[i] > 0.5 && xTune[i] < 1.0) {         //weiyy
						ySum += yTune[i];
						yList.add(new Double(yTune[i]));
						yCounter++;						
					}
