	...
	// what to do when a new set of data is prescribed:
    	...
    	scanController.addStartButtonListener(new ActionListener(){
    		public void actionPerformed(ActionEvent evn){
    			//NaWang
//    			try {
//    				((RfCavity) theDoc.myWindow().getCavityList().getSelectedValue()).setCavAmp(theDoc.getCavityDesignAmp());
...
//    			}
//    			try {
//    				System.out.println("Cavity state2 set to designvalue: "+((RfCavity) theDoc.myWindow().getCavityList().getSelectedValue()).getCavAmpAvg());
...
//    			}
       ...
       scanController1D.addNewSetOfDataListener(new ActionListener(){
    	...
    	scanController1D.addStartListener(new ActionListener(){
    		public void actionPerformed(ActionEvent evn){
    			...
    			try {
    				try {
    					theDoc.theCavity.setCavAmp(0);
    				} catch (PutException e) {
    					e.printStackTrace();
    				}
    				System.out.println("Cavity state1 set to 0: "+Double.toString(theDoc.theCavity.getCavAmpAvg()));
    			} catch (ConnectionException e) {
    				e.printStackTrace();
    			} catch (GetException e) {
    				e.printStackTrace();
    ...
    //			try {
    //				((RfCavity) theDoc.myWindow().getCavityList().getSelectedValue()).setCavAmp(0);
    //			} catch (ConnectionException e1) {
    //				e1.printStackTrace();
    //			} catch (PutException e1) {
    //				e1.printStackTrace();
    //			}
    ////			((RfCavity) theDoc.myWindow().cavityList.getSelectedValue()).updateDesignAmp(0);
    //			try {
    //				System.out.println("Cavity state1 set to 0tryagain: "+((RfCavity) theDoc.myWindow().getCavityList().getSelectedValue()).getCavAmpAvg());
    //			} catch (ConnectionException e) {
    //				e.printStackTrace();
    //			} catch (GetException e) {
    //				e.printStackTrace();
    //			}
        ...
        graphScan1D.setName("FCT Values vs. Cavity Phase, Cav Off");
    ...
    protected void updateScanVariables(FCT bpm1, FCT bpm2, RfCavity cav, CurrentMonitor bcm) {
    	...
    	BPM1AmpPVName = bpm1.getChannel(FCT.AMP_AVG_HANDLE).getId();
    	BPM1PhasePVName = bpm1.getChannel(FCT.PHASE_AVG_HANDLE).getId();
    	BPM2AmpPVName = bpm2.getChannel(FCT.AMP_AVG_HANDLE).getId();
    	BPM2PhasePVName = bpm2.getChannel(FCT.PHASE_AVG_HANDLE).getId();
    	...
    	scanVariable.setChannelRB(cav.getChannel(RfCavity.CAV_PHASE_AVG_HANDLE));
    	scanVariableParameter.setChannel(cav.getChannel(RfCavity.CAV_AMP_SET_HANDLE));
    	scanVariableParameter.setChannelRB(cav.getChannel(RfCavity.CAV_AMP_AVG_HANDLE));
    	BPM1PhaseMV.setChannel(bpm1.getChannel(FCT.PHASE_AVG_HANDLE));
    	BPM1AmpMV.setChannel(bpm1.getChannel(FCT.AMP_AVG_HANDLE));
    	BPM2PhaseMV.setChannel(bpm2.getChannel(FCT.PHASE_AVG_HANDLE));
    	BPM2AmpMV.setChannel(bpm2.getChannel(FCT.AMP_AVG_HANDLE));
    	cavAmpRBMV.setChannel(cavAmpRBChan);
    	BCMMV.setChannel(BCMChan);
    
    	BPM1PhaseOffMV.setChannel(bpm1.getChannel(FCT.PHASE_AVG_HANDLE));
    	BPM2PhaseOffMV.setChannel(bpm2.getChannel(FCT.PHASE_AVG_HANDLE));
    	...
    	// set the limitation of the cavity amplitude
        try {
            double amprb = theDoc.getCavity().getCavAmpAvg();
            double phaserb = theDoc.getCavity().getCavPhaseAvg();
            
            scanController.setParamLowLimit(amprb);
            scanController.setParamUppLimit(amprb);
            scanController.setParamCurrentValueRB(amprb);
            scanController.setCurrentValueRB(phaserb);
            
            scanController1D.setCurrentValueRB(phaserb);
    ...
    private void connectChannels(FCT bpm1, FCT bpm2, RfCavity cav) {
	    connectionMap.clear();
	    ...
	    bpm1.getChannel(FCT.PHASE_AVG_HANDLE).addConnectionListener(this);
	    bpm2.getChannel(FCT.PHASE_AVG_HANDLE).addConnectionListener(this);
	    bpm1.getChannel(FCT.AMP_AVG_HANDLE).addConnectionListener(this);
	    bpm2.getChannel(FCT.AMP_AVG_HANDLE).addConnectionListener(this);
	    ...
	    bpm1.getChannel(FCT.PHASE_AVG_HANDLE).requestConnection();
	    bpm2.getChannel(FCT.PHASE_AVG_HANDLE).requestConnection();
	    bpm1.getChannel(FCT.AMP_AVG_HANDLE).requestConnection();
	    bpm2.getChannel(FCT.AMP_AVG_HANDLE).requestConnection();
	    ...
	    while (nDisconnects > 0 && i < 5) {
    		...
    		nDisconnects = 0;
    		for(Boolean tf : connectionMap.values()){
    ...
    */
	...
	if (BPM1PhaseOffMV == null || BPM1PhaseOffMV.getDataContainer(0) == null) {
	    Toolkit.getDefaultToolkit().beep();
	    String errText = "Requested FCT1 phase with cav off data does not exist, ";
	...
	if (BPM1CavOffBGD.getNumbOfPoints() < 1) {
	    Toolkit.getDefaultToolkit().beep();
	    String errText = "Requested FCT1 phase with cav off data has too few points, ";
	    theDoc.myWindow().errorText.setText(errText);
	    System.err.println(errText);
	    return 0.;		
	}
    ...
    */
	...
	if(BPM2PhaseOffMV == null || BPM2PhaseOffMV.getDataContainer(0) == null) {
	    Toolkit.getDefaultToolkit().beep();
	    String errText = "Requested FCT2 phase with cav off data does not exist, ";
	    theDoc.myWindow().errorText.setText(errText);
	    System.err.println(errText);
	    return 0.;		
	}
	...
	if( BPM2CavOffBGD.getNumbOfPoints() < 1) {
	    Toolkit.getDefaultToolkit().beep();
	    String errText = "Requested FCT2 phase with cav off data has too few points, ";
	    theDoc.myWindow().errorText.setText(errText);
	    System.err.println(errText);
	    return 0.;		
	}
