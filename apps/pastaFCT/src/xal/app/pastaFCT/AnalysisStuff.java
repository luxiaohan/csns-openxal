...
import xal.extension.fit.spline.CubicSpline;
...
import xal.tools.data.DataTable.NonUniqueRecordException;
...
public class AnalysisStuff {
    ...
    * it may be further filtered for analysis purposes later */
    ...
    protected FCT firstBPM, secondBPM;
   ...
   // protected Hashtable<String, ParameterProxy> variableList = new Hashtable<String, ParameterProxy>();
   ...
   /** the default filename for the probe file */
    ...
    protected ArrayList<Boolean> variableActiveFlags = new ArrayList<Boolean>();
    ...
    protected void init() {
	    ...
	    if(!useCavOffData && (!theDoc.myWindow().useBPM1Box.isSelected() || !theDoc.myWindow().useBPM2Box.isSelected() ) ) {
    		...
    		String errText = "Cavity Off data must exist to use only 1 FCT";
    		theDoc.myWindow().errorText.setText(errText);
    		System.err.println(errText);
    		Application.displayError("Pasta Information", "Cavity Off data must exist to use only 1 FCT!");
    		return;
	    }
	    updateAnalysisData();
    ...
    */
    protected void updateAmpFactors() {
	    if(amplitudeVariableIndex < 0) {
	        ...
	        theDoc.myWindow().errorText.setText(errText);
	        System.err.println(errText);
	        Application.displayError("Pasta Information", "Please pick a scan number for the voltage variable to correspond to!");
	        return;
	    }
	    
	    // check for sanity:
	    if(amplitudeVariableIndex > nParamAmpVals-1) amplitudeVariableIndex = nParamAmpVals-1;
	    ampValueV.clear();
	    ...
	    for (int i=0; i<nParamAmpVals; i++) {
		    ...
		    if(i == amplitudeVariableIndex) ampValueV.add(new Double(cavityVoltage));
		    else {
		        double ratio = 0.;
    ...
    */    
    protected void updateAnalysisData() {
	    ...
	    if( theDoc.scanStuff.BPM2PhaseMV.getNumberOfDataContainers() != nParamAmpVals ||
			...
			Toolkit.getDefaultToolkit().beep();
			String errText = "Opps, The number of parametric scans is different for the same of the FCT amp. and phases\nDid the scan complete?";
			theDoc.myWindow().errorText.setText(errText);
			System.err.println(errText);
			Application.displayError("Pasta Information", "The number of parametric scans is different for the same of the FCT amp. and phases\nDid the scan complete?");
			return;
	     }
	     ...
	     for (int i=0; i<nParamAmpVals; i++) {	
			...
			if(bgdBPMAmp1.getNumbOfPoints() != bgdBPMAmp2.getNumbOfPoints()  ||
				...
				Toolkit.getDefaultToolkit().beep();
				String errText = "The number of points a parametric scan is not the same for FCT amp. and phases\nDid the scan complete?";
				theDoc.myWindow().errorText.setText(errText);
				System.err.println(errText);
				Application.displayError("Pasta Information", "The number of points a parametric scan is not the same for FCT amp. and phases\nDid the scan complete?");
				return;
			}
		     ...
		     // make fits of the measured BPM diff and amplitude to use later
		     // stash them
		     if(cavPhaseV.size() < 5) {
    			 Toolkit.getDefaultToolkit().beep();
    			 String errText = "Hmm - there do not appear to be enough good points to analyze!";
    			 theDoc.myWindow().errorText.setText(errText);
    			 System.err.println(errText);
    			 Application.displayError("Pasta Information", "There do not appear to be enough good points to analyze!");
    			 return;
		     }
    ...
    private void clearData() {  
	   ...
	   if(!theDoc.myWindow().useBPM1Box.isSelected() && !theDoc.myWindow().useBPM2Box.isSelected()) {
		    Toolkit.getDefaultToolkit().beep();
		    String errText = "Hey dude - you gotta select at least 1 FCT to analyze";
		    theDoc.myWindow().errorText.setText(errText);
		    System.err.println(errText);			
		    return;		   
	   }
    ...
    private double getPhase( ProbeState<? extends ProbeState<?>> state, FCT bpm) {
	    ...
	    double dt = ( bpm.getFCTBucket().getOrientation() * bpm.getFCTBucket().getLength()) / (2*IConstants.LightSpeed * beta);
	    double phase = (state.getTime() + dt) * 2. * Math.PI * freq;
	    phase = (phase % (2. * Math.PI)) * rad2deg;
	    return phase;
    }
    
     private double getPhaseAbs(double time, FCT bpm) {
	    double freq = bpm.getFCTBucket().getFrequency() * 1.e6;
    ...
    private double getPhase(double time, FCT bpm) {
	    double freq = bpm.getFCTBucket().getFrequency() * 1.e6;
	    double phase = (time) * 2. * Math.PI * freq;
	    phase = (phase % (2. * Math.PI)) * rad2deg;
	    return phase;
    ...
    private double getTime(ProbeState<?> state, FCT bpm) {
	    ...
	    double gamma = 1. + state.getKineticEnergy()/state.getSpeciesRestEnergy();
	    double beta = Math.sqrt(1. - 1./(gamma*gamma));
	    double dt = ( bpm.getFCTBucket().getOrientation() * bpm.getFCTBucket().getLength()) / (IConstants.LightSpeed * beta);	    
    ...
    */
    private void cavOffCalc() {
	   ...
	   // ProbeState[] states;
	    ...
	    RfCavity theCavity = theDoc.getCavity();
	    
	    theCavity.updateDesignAmp(0.); // turn off the cavity
	    theCavity.updateDesignPhase(0.);// cavity phase setting does not matter
	    ...
	    try{
		    ...
		    states = traj.statesForElement(theDoc.getFCT1().getId());
		    //state1 = (EnvelopeProbeState) states[0];
		    //state1 = (ProbeState) states[0];
		    state1 = states.get(0);
		    
		    states = traj.statesForElement(theDoc.getFCT2().getId());
		    //state2 = (EnvelopeProbeState) states[0];
		    //state2 = (ProbeState) states[0];
		    state2 = states.get(0);
	    
		    BPM1TimeCavOff = getTime(state1, theDoc.getFCT1()) - time0;
		    BPM2TimeCavOff = getTime(state2, theDoc.getFCT2()) - time0;
	    }
	    catch(Exception exc) {
    ...
    /* run the model for a given phase, amplitude and beam input energy */
    private void runModel(double amp, double phase, double energy) {
        RfCavity theCavity = theDoc.getCavity();
        theCavity.updateDesignAmp(amp);
        theCavity.updateDesignPhase(phase);
        try{
            theModel.resync();
        }
        catch (Exception exc) {
   ...
   /** do a single model pass - scan through the phases for the prescribed 
    ...
    */
    private void singlePass(int i) {
	    ...
	    double cavPhase, cavPhaseScaled,  phase1, phase2, diff;
//	    double betaZ1, betaZ2;
//	    double sigmaz1, sigmaz2;
	    double WOut;
...
//	    Twiss[] twiss1, twiss2;
    ...
    public double doCalc() {
	    ...
	    if(useOneBPM) {
		    ...
		    if (theDoc.myWindow().useBPM1Box.isSelected() ) {
			   firstBPM = theDoc.getFCT1();
			   secondBPM = theDoc.getFCT1();
		    }
		    else {
			   firstBPM = theDoc.getFCT2();
			   secondBPM = theDoc.getFCT2();    
		    }
	    }
	    else {
		    firstBPM = theDoc.getFCT1();
	    ...
	    haveData = true;
	    theDoc.myWindow().errorField.setText(theDoc.myWindow().prettyString(errorTotal));
	    return errorTotal;
    }
    ...
    * i.e. sqrt(sum residuals)*/
    protected  void solve() {
	     /*solverInit();
	   System.out.println("Starting to solve, alg = " + solver.getSearchAlgorithm().getClass() );
	    solver.solve();
    ...
    private boolean calcInit() {
	    Collection<AcceleratorNode> cavs2;
		...
		if ((theDoc.theSequence.getClass()).equals(AcceleratorSeqCombo.class)){
			...
			while (itr.hasNext()) {
				...
				//NaWang
				double positiondiff=theDoc.theSequence.getDistanceBetween(theDoc.getCavity(), seq);
				if(!seq.getId().equals(theDoc.getCavity().getId()) && positiondiff > 0.0) {
					((RfCavity) seq).updateDesignAmp(0.);
					System.out.println("deactivating cavity: " + seq.getId());
				}
				//NaWang
			}
		}
...
//    	// run only through part of sequence of interest:
    	theProbe.getAlgorithm().setStopElementId(theDoc.getCavity().getId());
	    ...
	    if(theProbe == null) {
		    ...
		    if(probeFileName != null) {
    			try{
    				...
    				theDoc.myWindow().errorText.setText(errText);
    				System.err.println(errText);
    				Application.displayError("Pasta Information", "Can't read the probe file you requested: \"" + probeFileName + "\"!");
    				return false;
    			}
		    }
		    else{
			    ...
			    */
			    ...
			    String cavname = theDoc.getCavity().getId();
			    final EditContext editContext = theDoc.theSequence.getAccelerator().editContext();
		        final DataTable locationTable = editContext.getTable("location");
		        
		        //寻找与cavity相关的probe参数，如果model.params文件中没有设定，则使用这段sequence的probe
			    try {
					if(locationTable.record( "name", cavname) == null)
					    //theProbe = ProbeFactory.getParticleProbe(theDoc.theSequence, pt);
					    theProbe = ProbeFactory.createParticleProbe(theDoc.theSequence, AlgorithmFactory.createParticleTracker(theDoc.theSequence));
					else
					    theProbe = ProbeFactory.getParticleProbe(theDoc.getCavity().getId(), theDoc.theSequence, pt);
				} catch (NonUniqueRecordException | InstantiationException e) {
    ...
    /** set up the solver variables for the given problem */
   /* private void solverInit() {
	    variableList.clear();
	    
	    if( ((Boolean) variableActiveFlags.get(1)).booleanValue()) {
		    ParameterProxy varWIn = new ParameterProxy(WIN_VAR_NAME, WIn, WIn*0.007, WIn*0.975, WIn*1.025);
		    variableList.put("WIn",varWIn);
	    }
	    
	    if( ((Boolean) variableActiveFlags.get(0)).booleanValue()) {
		    ParameterProxy varCavPhaseOffset = new ParameterProxy(PHASE_VAR_NAME, cavPhaseOffset, 1., cavPhaseOffset-20.,cavPhaseOffset+20.);
	    ...
	    if( ((Boolean) variableActiveFlags.get(2)).booleanValue()) {	   
		    ...
		    ParameterProxy pp = new ParameterProxy(AMP_VAR_NAME, val, 0.01, val*0.85, val*1.15);
		    variableList.put("AmpFac", pp);
	    }
	    
	    if( ((Boolean) variableActiveFlags.get(3)).booleanValue()) {
		   ParameterProxy varFF = new ParameterProxy(FF_VAR_NAME, theDoc.fudgePhaseOffset, 1., -180, 180);
		   variableList.put("Fudge", varFF);		    
	    }
	    
	    algorithmId = (String) theDoc.myWindow().algorithmChooser.getSelectedItem();
	    if(algorithmId.equals(RANDOM_ALG) ) {
		    algorithm = new RandomSearchAlgorithm();
	    }
	    else if(algorithmId.equals(SIMPLEX_ALG) ) {
		    algorithm = new SimplexSearchAlgorithm();
	    }
	    else if(algorithmId.equals(POWELL_ALG) ) {
		    algorithm = new PowellSearchAlgorithm();
		    int oneD_iterations = 10;
		    ((PowellSearchAlgorithm) algorithm).setMaxIterationFor1D(oneD_iterations);
	    }
	    else {
	 	    algorithm = new RandomSearchAlgorithm();
	    }
    ...
    protected boolean setProbe(File file) {
	    ...
	    try {
		    ...
		    if(theDoc.myWindow() != null) theDoc.myWindow().errorText.setText(errText);
		    System.err.println(errText);
		    Application.displayError("Pasta Information", "Can't pasre the probe file: \"" + file.getPath() + "\"!");
		    return false;		    
	    }	  
    ...
    protected void updateSetpoints() {
	    ...
	    cavPhaseSetpoint = theDoc.getCavityDesignPhase() + cavPhaseOffset - theDoc.DTLPhaseOffset;
	    if(svs.length < 2) {
	    ...
	    int ind = 0;
	    while(theDoc.getCavityDesignAmp() <= svs[ind] && ind < svs.length-1) {
		    ind++;
	    }
	    if(ind > (pvs.length -2)) ind = pvs.length -2;
	    double slope = (pvs[ind+1] - pvs[ind])/(svs[ind+1]-svs[ind]);
	    cavAmpSetpoint = pvs[ind] + (theDoc.getCavityDesignAmp() - svs[ind])*slope;
            
            runModel(cavityVoltage, theDoc.getCavityDesignPhase(), WIn*1.e6);
            /*Trajectory traj = theModel.getProbe().getTrajectory();
            ProbeState[] states =traj.statesForElement(firstBPM.getId()); 
            ProbeState state = (ProbeState) states[0];*/
...
/*				                                
    ...
    protected void  sendNewSetpoints() {
	    ...
	    try {
		    theDoc.getCavity().setCavPhase(cavPhaseSetpoint);
		    theDoc.getCavity().setCavAmp(cavAmpSetpoint);
	    } catch (Exception ex) {
		Toolkit.getDefaultToolkit().beep();
    ...
    protected double[] toDouble(Vector<Double> vec) {
    	...
    	for (int i=0; i< da.length; i++) da[i] = vec.get(i);
    ...
    */
    ...
    protected void initialGuess() {
        ...
        WIn = defaultEnergy;
        // same with amplitude:
        cavityVoltage = theDoc.getCavityDesignAmp();
        //assume the cavity phase setpoint is in the center of the scan range:
        if(theDoc.myWindow().useBPM1Box.isSelected()) someMeasuredData = theDoc.scanStuff.BPM1AmpMV.getDataContainer(0); 
        if(theDoc.myWindow().useBPM2Box.isSelected()) someMeasuredData = theDoc.scanStuff.BPM2AmpMV.getDataContainer(0); 
        if(someMeasuredData == null) {
            String errText = "Hey - give me some measured data first!!!: ";
            Toolkit.getDefaultToolkit().beep();			
			theDoc.myWindow().errorText.setText(errText);
			System.err.println(errText);
			Application.displayError("Pasta Information", "Measure data is needed!");
            return;
        }
        ...
        System.out.println("Phases range: (" + minPhase + ", " + maxPhase + ")");
        
        cavPhaseOffset = avgPhase - theDoc.getCavityDesignPhase()  + theDoc.DTLPhaseOffset;
            
        // give some margin away from the scan endpoints for analysis range:
        phaseModelMax = maxPhase - 0.1 * dPhase;
        theDoc.myWindow().maxScanPhaseField.setValue(phaseModelMax);
        phaseModelMin = minPhase + 0.1 * dPhase;
     ...
     * @param scenario
     */
    public void setModel(Scenario scenario){
	...
	private class PastaScorer implements Scorer {
		...
		/*public double score() {
			...
			if(variableList.get("WIn") != null) WIn = ((ParameterProxy) variableList.get("WIn")).getValue();
			if(variableList.get("PhaseOffset") != null) cavPhaseOffset = ((ParameterProxy) variableList.get("PhaseOffset")).getValue();
			if(variableList.get("AmpFac") != null) { 
				//ParameterProxy pp = (ParameterProxy) variableList.get("AmpFac");
				Variable pp =  variableList.get("AmpFac");
				cavityVoltage = pp.getValue();
				updateAmpFactors();
			}
			
			if(variableList.get("Fudge") != null)  { 
				theDoc.fudgePhaseOffset = ((ParameterProxy) variableList.get("Fudge")).getValue();
			}
