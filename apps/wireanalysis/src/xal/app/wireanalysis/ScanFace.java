...
public class ScanFace extends JPanel {
	...
	public void addComponents() {
		...
		layout.add(sequencebutton, mainPanel, 5, 10, EdgeLayout.LEFT);
				...
				for (int i = 0; i < wiretablemodel.getRowCount(); i++) {
					if ((Boolean) wiretablemodel.getValueAt(i, 1)) {
						scanwirelist.add(new WireScanner(
								(String) wiretablemodel.getValueAt(i, 0)));
						System.out.println("activating wire for "
								+ scanwirelist.get(i));
					}
				...
				for (int i = 0; i < wiretablemodel.getRowCount(); i++) {
					if ((Boolean) wiretablemodel.getValueAt(i, 1)) {
						abortwirelist.add(new WireScanner(
								(String) wiretablemodel.getValueAt(i, 0)));
						System.out.println("aborting wire for "
								+ abortwirelist.get(i));
					}
	...
	private void startScans(ArrayList<WireScanner> scanwires) {
		...
		while (itr.hasNext()) {
			...
			try {
				...
				System.out.println("Can not put run command arguments for "
						+ ws.getId());
