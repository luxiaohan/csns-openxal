...
public class ModelFace extends JPanel {
    ...
    double emittancelocx0, alphalocx0, betalocx0;
    double emittancelocy0, alphalocy0, betalocy0;
    double emittancelocx, alphalocx, betalocx;
    double emittancelocy, alphalocy, betalocy;
	...
	public ModelFace(GenDocument aDocument) {
		...
		double alphax = currenttwiss[0];
		double betax = currenttwiss[1];
		double alphay = currenttwiss[3];
		double betay = currenttwiss[4];
	...
	public void addComponents() {
		...
		layout.setConstraints(resultsPanel, 120, 450, 10, 10, EdgeLayout.LEFT,
		...
		layout.add(plotellipsebutton, mainPanel, 230, 650, EdgeLayout.LEFT);//liyong add
		this.add(mainPanel);
	}
	...
	public void makeComponents() {
		...
		plotpanel.setBorder(BorderFactory
				.createTitledBorder("Raw Data Display"));
		...
		plotellipsebutton = new JButton("Plot Phase plane Ellipse");//liyong add
		plotellipsebutton.setBackground(Color.pink);//liyong add
	...
	public void setAction() {
		...
		selectSequence.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				// zp add
				for (int i = 0; i < seqlistnames.size(); i++) {
				...
				// zp delete
				/*
				 * while(itr.hasNext()){ System.out.println(itr.next()); String
				 * seqname= (String)itr.next(); AcceleratorSeq sequence =
				 * accl.getSequence(seqname);
				 * System.out.println("sequence:"+sequence.getId());
				 * seqlist.add((AcceleratorSeq)sequence);
				 * nodes.addAll(sequence.getAllNodes(true)); }
				 */
		...
		loadbutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (pvloggerid.intValue() == 0) {
					...
					machinestatechooser.setSelectedItem(new String(
							"User Defined"));
				} else {
		...
		// ((String)pvloggerfield.getText() != null)){
		// String id = pvloggerfield.getText();
		// pvloggerid = new Integer(Integer.parseInt(id));
		// doc.currentpvloggerid = pvloggerid;
		// doc.masterpvloggermap.put((String)datatablemodel.getValueAt(0,0),
		// (Integer)pvloggerid);
		...
		elementList.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				modelinitialized = false;
				probeeditbutton.setEnabled(false);
				System.out.println("Changing to new element :"
						+ (String) elementList.getSelectedItem());
				errorlabel
						.setText("WARNING: Model probe set to default values when element is changed! ");
			}
		});
		...
		average.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < storedtable.getRowCount(); i++)
					if ((Boolean) storedtable.getValueAt(i, 8)) {
						axdev += ((Double) storedtable.getValueAt(i, 2) - (axsum / count))
								* ((Double) storedtable.getValueAt(i, 2) - (axsum / count));
						aydev += ((Double) storedtable.getValueAt(i, 3) - (aysum / count))
								* ((Double) storedtable.getValueAt(i, 3) - (aysum / count));
						bxdev += ((Double) storedtable.getValueAt(i, 4) - (bxsum / count))
								* ((Double) storedtable.getValueAt(i, 4) - (bxsum / count));
						bydev += ((Double) storedtable.getValueAt(i, 5) - (bysum / count))
								* ((Double) storedtable.getValueAt(i, 5) - (bysum / count));
						exdev += ((Double) storedtable.getValueAt(i, 6) - (exsum / count))
								* ((Double) storedtable.getValueAt(i, 6) - (exsum / count));
						eydev += ((Double) storedtable.getValueAt(i, 7) - (eysum / count))
								* ((Double) storedtable.getValueAt(i, 7) - (eysum / count));
					}
		...
		// (String)datatablemodel.getValueAt(0,0) + " with " +pvloggerid);
		// doc.masterpvloggermap.put((String)datatablemodel.getValueAt(0,0),
		// (Integer)pvloggerid);
		// }
		// else{
		...
		// }
		// }
		// if(!selectedfile.equals(new String("User Defined")) || id.equals(new
		// String(""))){
		// pvloggerid = (Integer)pvloggermap.get(selectedfile);
		// System.out.println("Taking pvlogger from file " + selectedfile +
		// "; pvloggerid is = " + pvloggerid);
		// doc.currentpvloggerid = pvloggerid;
		//
		// }
		// }
		...
		/*probeeditbutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				System.out.println("In probe and beam current is "
						+ initprobe.getBeamCurrent());
		...
		//**************liyong add on 2016-3-16*********************
		plotellipsebutton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				GenWindow mywindow=(GenWindow) doc.getMainWindow();
                EllipseDialog dialog=new EllipseDialog(mywindow);
                dialog.setTwisParamitersX0(emittancelocx0, alphalocx0, betalocx0);
                dialog.setTwisParamitersX(emittancelocx, alphalocx, betalocx);
                dialog.setTwisParamitersY0(emittancelocy0, alphalocy0, betalocy0);
                dialog.setTwisParamitersY(emittancelocy, alphalocy, betalocy);
                dialog.setLocationRelativeTo(mywindow);
                dialog.setVisible(true);
              //  System.out.println("emittancelocx0: "+emittancelocx0);
              //  System.out.println("alphalocx0: "+alphalocx0);
              //  System.out.println("betalocx0: "+betalocx0);
			}
		});
		//**********************************************************
		setlimits.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				alphaXlower.setText(numfor.format(alphaXmin));
				alphaXupper.setText(numfor.format(alphaXmax));
				alphaYinitial.setText(numfor.format(alphay0));
				alphaYlower.setText(numfor.format(alphaYmin));
				alphaYupper.setText(numfor.format(alphaYmax));
				betaXinitial.setText(numfor.format(betax0));
				betaXlower.setText(numfor.format(betaXmin));
				betaXupper.setText(numfor.format(betaXmax));
				betaYinitial.setText(numfor.format(betay0));
				betaYlower.setText(numfor.format(betaYmin));
				betaYupper.setText(numfor.format(betaYmax));
				emitXinitial.setText(numfor.format(emitx0));
				emitXlower.setText(numfor.format(emitXmin));
				emitXupper.setText(numfor.format(emitXmax));
				emitYinitial.setText(numfor.format(emity0));
				emitYlower.setText(numfor.format(emitYmin));
				emitYupper.setText(numfor.format(emitYmax));
				 ...
				 * EnvelopeProbe tempprobe=ProbeFactory.getEnvelopeProbe(seq,
				 * etracker); tempprobe =
				 * EnvelopeProbe.newProbeInitializedFrom(initprobe); Scenario
				 * tempscenario; try{ tempscenario =
				 * Scenario.newScenarioFor(seq);
				 * tempscenario.setProbe(tempprobe);
				 * tempscenario.setSynchronizationMode
				 * (Scenario.SYNC_MODE_DESIGN); PVLoggerDataSource plds = new
				 * PVLoggerDataSource(pvloggerid.intValue()); tempscenario =
				 * plds.setModelSource(seq, tempscenario);
				 * tempscenario.resync(); tempscenario.run();
				 * plds.closeConnection(); }catch(Exception ex){}
				 * 
				 * EnvelopeTrajectory traj = (EnvelopeTrajectory)
				 * tempprobe.getTrajectory(); Iterator iterState=
				 * traj.stateIterator(); EnvelopeProbeState state =
				 * (EnvelopeProbeState)iterState.next(); Twiss[] twiss =
				 * state.getTwiss();
				 */
				limitdialog.add(set);
				limitdialog.pack();
	...
	public void buildseq() {
		...
		/*
		 * int startp =
		 * findSeqp((ArrayList)accl.getSequences(),(String)elementList
		 * .getSelectedItem()); int endp =
		 * findSeqp((ArrayList)accl.getSequences(),(String)Wirenamelist.get(0));
		 * for(int i=1;i<Wirenamelist.size();i++){
		 * if(findSeqp((ArrayList)accl.getSequences
		 * (),(String)Wirenamelist.get(i))>endp) endp =
		 * findSeqp((ArrayList)accl.getSequences(),(String)Wirenamelist.get(i));
		 * } seq = new AcceleratorSeqCombo("Combo",
		 * accl.getSequences().subList(startp,endp+1));
		 */
		seq = solveforseq;
	...
	public void updateStored() {
		...
		if (!((String) machinestatechooser.getSelectedItem())
				.equals("User Defined")) {
			storeddata.add(new String((String) machinestatechooser
					.getSelectedItem()));
		}
	...
	public void plotfromstorage() {
		...
		for (int i = 0; i < storedtable.getRowCount(); i++) {
			if ((Boolean) storedtable.getValueAt(i, 8)) {
				if (graphChooser.getSelectedIndex() == 3
						|| graphChooser.getSelectedIndex() == 5) {
					BasicGraphData point = new BasicGraphData();
					point.addPoint((Double) storedtable.getValueAt(i, 2),
							(Double) storedtable.getValueAt(i, 4));
					plotGraph(point);
				}
				if (graphChooser.getSelectedIndex() == 4
						|| graphChooser.getSelectedIndex() == 5) {
					BasicGraphData point = new BasicGraphData();
					point.addPoint((Double) storedtable.getValueAt(i, 3),
							(Double) storedtable.getValueAt(i, 5));
					plotGraph(point);
				}
	...
	public boolean errorcheck() {
		...
		for (int i = 0; i < datatable.getRowCount(); i++) {
			if ((Boolean) datatable.getValueAt(i, 6)
					&& ((findNodep(seq, (String) datatable.getValueAt(i, 1)) < findNodep(
							seq, (String) elementList.getSelectedItem())))) {
				errorlabel
						.setText("Please only select an element upstream from the first Wire");
				return true;
			}
		}
		return false;
	...
	public void selectSequences() {
		...
		for (int i = 0; i < rows; i++) {
			...
			if (currentwire.startsWith("MEBT") && useMEBT == false)
				useMEBT = true;
			if (currentwire.startsWith("DTL") && useDTL == false)
				useDTL = true;
			if (currentwire.startsWith("LRBT") && useLRBT == false)
				useLRBT = true;
			if (currentwire.startsWith("RTBT") && useRTBT == false)
				useRTBT = true;
		}
	}
		...
		if (edmresultsdatatable.records().size() == 0) {
			...
			while (itr.hasNext()) {
				...
				if (fitresultsdatatable.records().size() == 0) {
					...
					GenericRecord hfitrecord = fitresultsdatatable
							.record(hbindings);
					if (hfitrecord == null) {
						tabledata.add(new Double(0.0));
					} else {
	...
	public void refreshFileArray() {
		...
		for (int i = 0; i < rows; i++) {
			...
			if (!filesloaded.contains(filename)) {
				...
				machinestatechooser.addItem((String) datatablemodel.getValueAt(
						i, 0));
			}
		}
	...
	void initModel() {
		...
		try {
			...
			// zp delete
			// PVLoggerDataSource plds = new
			// PVLoggerDataSource(pvloggerid.intValue());
	...
	public void singlepass() {
		...
		// want to copy the tracker if we can to avoid editing the shared probe
		// tracker elsewhere
		...
		System.out.println("Starting with ax, bx, ex = " + alphax0 + " "
				+ betax0 + " " + emitx0);
		System.out.println("Starting with ay, by, ey = " + alphay0 + " "
				+ betay0 + " " + emity0);
		System.out.println("Starting with az, bz, ez = " + alphaz0 + " "
				+ betaz0 + " " + emitz0);
	...
	public void solve() {
		...
		// zp add
		// System.out.println("betaXmax: "+betaXmax);
		...
		// Try different Solvers to get the initial Twiss parameters.
		Stopper maxSolutionStopper = SolveStopperFactory
				.minMaxTimeSatisfactionStopper(1,
						Integer.parseInt(timefield.getText()), 0.999);
		// Solver solver = new Solver(new DirectedStep(),maxSolutionStopper );
		// Solver solver = new Solver(new
		// RandomShrinkSearch(),maxSolutionStopper );//zpdelete
		Solver solver = new Solver(new SimplexSearchAlgorithm(),
				maxSolutionStopper);
		...
		resetEllipseTwiss(variables, best);//liyong add on 2016-3-16
	...
	private void resetEllipseTwiss(ArrayList vars, Trial trial) {//liyong add on 2016-3-16
		...
		String beginElement ="Begin_Of_"+seq.getEntranceID().toString();
		EnvelopeProbeState state1 = (EnvelopeProbeState) traj.statesForElement(beginElement);
        CovarianceMatrix covarianceMatrix1 = state1.getCovarianceMatrix();
        Twiss[] twiss1 = covarianceMatrix1.computeTwiss();
		alphalocx0=twiss1[0].getAlpha();		
		betalocx0=twiss1[0].getBeta();
		emittancelocx0=twiss1[0].getEmittance()*1e6;
		
		alphalocy0=twiss1[1].getAlpha();
		betalocy0=twiss1[1].getBeta();
		emittancelocy0=twiss1[1].getEmittance()*1e6;
		
		String slitElement = null;
		System.out.println("seq.getId(): "+seq.getEntranceID());
		if(seq.getEntranceID().equals("MEBT")){
			slitElement ="MEEM01";	
		}	
		 EnvelopeProbeState state2 = (EnvelopeProbeState) traj.statesForElement(slitElement);
	     CovarianceMatrix covarianceMatrix2 = state2.getCovarianceMatrix();
	     Twiss[] twiss2 = covarianceMatrix2.computeTwiss();
		 alphalocx=twiss2[0].getAlpha();		
		 betalocx=twiss2[0].getBeta();
		 emittancelocx=twiss2[0].getEmittance()*1e6;	
		    
		 alphalocy=twiss2[1].getAlpha();
		 betalocy=twiss2[1].getBeta();
		 emittancelocy=twiss2[1].getEmittance()*1e6;
		    
	     System.out.println("0emittancelocx: "+emittancelocx);
	     System.out.println("0alphalocx: "+alphalocx);
	     System.out.println("0betalocx: "+betalocx);
		
	}

	// zp TODO more changes
	void setInitLimits() {

		// Set upper limits on alpha and beta to max values in sequence, lower
		// limit to zero or neg. max.
		...
		if (!limitsWereSet || elementchanged) {
			...
			// zp add
			// System.out.println("max/beta: "+betaYmax);
			...
			while (iterState.hasNext()) {
				...
				if (Math.abs(twiss[0].getAlpha()) > alphaXmax)
					alphaXmax = Math.abs(twiss[0].getAlpha());
				if (Math.abs(twiss[1].getAlpha()) > alphaYmax)
					alphaYmax = Math.abs(twiss[1].getAlpha());
				if (!RTBTlimit) {
					if (twiss[0].getBeta() > betaXmax)
						betaXmax = twiss[0].getBeta();
					if (twiss[1].getBeta() > betaYmax)
						betaYmax = twiss[1].getBeta();
					if (state.toString().contains("RTBT_Diag:WS24"))
						RTBTlimit = true;
				}
				if (twiss[0].getEmittance() > emitXmax)
					emitXmax = twiss[0].getEmittance() * 1e6;
				if (twiss[1].getEmittance() > emitYmax)
					emitYmax = twiss[1].getEmittance() * 1e6;
			}
			alphaXmax *= 1.0;
			alphaYmax *= 1.0;
	...
	public void resetTwissTable(ArrayList vars, Trial trial) {
		...
		while (itr.hasNext()) {
			...
			if (name.equalsIgnoreCase("alphaX")) {
				twisstablemodel.setValueAt(new Double(decfor.format(value)), 0,
						1);
			...
			if (name.equalsIgnoreCase("betaX")) {
				twisstablemodel.setValueAt(new Double(decfor.format(value)), 1,
						1);
			...
			if (name.equalsIgnoreCase("emitY")) {
				twisstablemodel.setValueAt(new Double(decfor.format(value)), 2,
						2);
				twisstablemodel.setValueAt(
						new Double(decfor.format(value * gammabeta)), 3, 2);
			...
			if (name.equalsIgnoreCase("emitX")) {
				twisstablemodel.setValueAt(new Double(decfor.format(value)), 2,
						1);
				twisstablemodel.setValueAt(
						new Double(decfor.format(value * gammabeta)), 3, 1);
	...
	public void makeTwissTable() {
		...
		twissscrollpane = new JScrollPane(twisstable,
	...
	public void makeStoredResultsTable() {
		String[] colnames = { "File Name", "Element solved for", "alphax",
				"alphay", "betax", "betay", "emitx", "emity", "Select" };
		...
		storedtable.setPreferredScrollableViewportSize(storedtable
				.getPreferredSize());
		...
		storedscrollpane = new JScrollPane(storedtable,
	...
	public void makeDataTable() {
		String[] colnames = { "File Name", "Wire", "Edm x_rms", "Edm y_rms",
				"User X_rms", "User Y_rms", "Select" };
