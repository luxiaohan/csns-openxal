...
import xal.app.beamatfoil.BeamAtFoilWindow;
import xal.app.beamatfoil.utils.LRBTOrbitController;
import xal.app.beamatfoil.utils.RingFoilPosController;
//import csns.apps.beamatfoil.utils.LRBTOrbitController;
import xal.ca.*;
...
//import csns.apps.beamatfoil.utils.*;
...
public class BeamAtFoilDocument extends AcceleratorDocument {
	...
	//setFont_PrefPanel_Button.setBounds(10,10,20,20);
	...
	//=====================================
	private LRBTOrbitController lrbtOrbitController	= null;
	...
	//------------------------------------------
	//root node name
	private String dataRootName = "BeamAtFoil of CSNS Ring";
	...
	public BeamAtFoilDocument() {
		...
		lrbtOrbitController = new LRBTOrbitController(updatingController);
		ringFoilPosController = new RingFoilPosController(updatingController);

		//set up tabbed panels
		firstPanel = lrbtOrbitController.getPanel();
		...
		mainTabbedPanel.add("LRBT Beam at Foil", firstPanel);
		...
		AcceleratorSeq lrbt = acc.getComboSequence("LRBT");
		lrbtOrbitController.setAccelSeq(lrbt);
	...
	public void makeMainWindow() {
		...
		lrbtOrbitController.getMessageText().setDocument(messageTextLocal.getDocument());
	...
	public void saveDocumentAs(URL url) {
		...
		try {
			beamatfoilData_Adaptor.writeTo(new File(url.getFile()));
			//super class method - will show "Save" menu active
			setHasChanges(true);
		} catch(IOException e) {
	...
	private void makePreferencesPanel() {
		...
		tmp_2.add(lrbtOrbitController.getSignXText());
		tmp_2.add(signCoeffX_Label);

		JPanel tmp_3 = new JPanel(new FlowLayout(FlowLayout.LEFT, 1, 1));
		tmp_3.add(lrbtOrbitController.getSignYText());
	...
	private void setFontForAll(Font fnt) {
		...
		lrbtOrbitController.setFontForAll(fnt);
