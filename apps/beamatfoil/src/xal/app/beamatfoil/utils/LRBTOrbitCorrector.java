 ...
 *
 *@author     shishlo
 *adjusted by qiujing of CSNS
 */
public class LRBTOrbitCorrector {

	//main panel
	private JPanel LRBTOrbitCorrectorMainPanel = new JPanel();
	...
	/**
	 *  Constructor for the LRBTOrbitCorrector object
	 */
	public LRBTOrbitCorrector(String borderTitle) {
		...
		LRBTOrbitCorrectorMainPanel.setLayout(new BorderLayout());
		corrTableBorder = BorderFactory.createTitledBorder(border, borderTitle);
		LRBTOrbitCorrectorMainPanel.setBorder(corrTableBorder);
    ...
    //set the fitting panel
		...
		posWheel.setFormat("+##.#");
		//posWheel.setBackground(Color.red);
		...
		LRBTOrbitCorrectorMainPanel.add(tablePanel, BorderLayout.WEST);
		LRBTOrbitCorrectorMainPanel.add(rightPanel, BorderLayout.CENTER);
	...
	/**
	 *  Returns the panel attribute of the LRBTOrbitCorrector object
	 *
	 *@return    The panel value
	 */
	public JPanel getPanel() {
		return LRBTOrbitCorrectorMainPanel;
	}
	...
	/**
	 *  Sets the accelSeq attribute of the LRBTOrbitCorrector object
	 *
	...
	/**
	 *  Sets the fontForAll attribute of the LRBTOrbitCorrector object
	 *
	...
	public void setFontForAll(Font fnt) {
		...
		//posWheel.setFont(fnt);
		posWheelLabel.setFont(fnt);
		//angleWheel.setFont(fnt);
	...
	*/
	...
	/**
	 *  Returns the messageText attribute of the LRBTOrbitCorrector object
	 *
	...
	/**
	 *  Sets the messageText attribute of the LRBTOrbitCorrector object
	 */
	 ...
	 private boolean setUpCorrectorsCoeff(){
		 ...
		 for(int i = 0, n = corrV.size(); i < n; i++) {
			...
			// PhaseMatrix phMatr = trajectory.stateResponse(corrElm.getName(),foil.getId());
			 ...
			 double mPos = 0.;
			 double mAngle = 0.;
			 if(corrElm.getName().indexOf(":LRDH") > 0) {
				 mPos = phMatr.getElem(0, 1);
				 mAngle = phMatr.getElem(1, 1);
			 }
			 if(corrElm.getName().indexOf(":LRDV") > 0) {
	...
	private boolean findCorrection() {
		...
		for(int i = 0, n = corrV.size(); i < n; i++) {
			...
			if(corrElm.isActive()){
				...
				//ParameterProxy proxy = new ParameterProxy(corrElm.getName(),initVal,upperLimit/20.0); 
		...
		Scorer scorer = new Scorer(){
			public double score(Trial trial, List<Variable> scoreVariables){
				...
				for(int i = 0, n = corrProxyV.size(); i < n; i++) {
					//ParameterProxy proxy = corrProxyV.get(i);
