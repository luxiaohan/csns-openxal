...
public class RingFoilPosCorrector {
	...
	private BPM bpmStart = null;
	private BPM bpmEnd = null;
	...
	private PhaseMatrix R4BPM01_Foil_phm = new PhaseMatrix();
	//BPM10 t0 BPM_A13 matrix
	private PhaseMatrix R4BPM01_R1BPM01_phm = new PhaseMatrix();	
	...
	private JLabel r4bpm01PosLabel =  new JLabel("<=x R1BPM01 [mm]",JLabel.LEFT);
	private JLabel r1bpm01PosLabel = new JLabel("<=x R4BPM01 [mm]",JLabel.LEFT);
	
	protected DoubleInputTextField r4bpm01PosTextField = new DoubleInputTextField(10);
	protected DoubleInputTextField r1bpm01PosTextField = new DoubleInputTextField(10);
	...
	public RingFoilPosCorrector(String borderTitle) {
		...
		correctionControlBorder = BorderFactory.createTitledBorder(border, "CSNS Beam at foil position control");
    ...
    //set the fitting panel
		...
		ftCnt0Panel.add(r4bpm01PosTextField);
		ftCnt0Panel.add(r4bpm01PosLabel);
		ftCnt0Panel.add(r1bpm01PosTextField);
		ftCnt0Panel.add(r1bpm01PosLabel);
	...
	public void setAccelSeq(AcceleratorSeq accSeq) {
		this.accSeq = accSeq;
		corrV.clear();
		HDipoleCorr corr = (HDipoleCorr)accSeq.getNodeWithId("RCS_Mag:R4DH01");
		//corr.getMagBucket().setBendAngle( 0.0 );
		Corr_Element corrElm = new Corr_Element(corr.getId(), corr);
		corrElm.setActive(true);
		corrV.add(corrElm);
		corr =(HDipoleCorr)accSeq.getNodeWithId("RCS_Mag:R1DH01");
		//corr.getMagBucket().setBendAngle( 0.0 );
	  ...
	  bpmStart = (BPM) accSeq.getNodeWithId("RCS_Diag:R4BPM01");
	  bpmEnd = (BPM) accSeq.getNodeWithId("RCS_Diag:R1BPM01");
	...
	public void setFontForAll(Font fnt) {
		...
		r4bpm01PosLabel.setFont(fnt);
		r1bpm01PosLabel.setFont(fnt);
		xFoilPosLabel.setFont(fnt);
		xpFoilPosLabel.setFont(fnt);
		r4bpm01PosTextField.setFont(fnt);
		r1bpm01PosTextField.setFont(fnt);
	 ...
	 */	
	 private boolean setUpCorrectorsCoeff(){
	   ...
	   makePhaseMatrix(trajectory);
		 ...
		 //x_start = 0.025;
		 //x_end = 0.020;
		 r4bpm01PosTextField.setValue(1000.*x_start);
		 r1bpm01PosTextField.setValue(1000.*x_end);		 

		 double a00 = R4BPM01_R1BPM01_phm.getElem(0,0);
		 double a01 = R4BPM01_R1BPM01_phm.getElem(0,1);
		 double a10 = R4BPM01_R1BPM01_phm.getElem(1,0);
		 double a11 = R4BPM01_R1BPM01_phm.getElem(1,1);
		 double c0 =  R4BPM01_R1BPM01_phm.getElem(0,6);
		 double c1 =  R4BPM01_R1BPM01_phm.getElem(1,6);
		 if(a01 == 0. || a11 == 0.){
		 ...
		 PhaseVector bpmEndPHV = R4BPM01_R1BPM01_phm.times(bpmStartPHV);
		 //System.out.println("debug phEnd="+bpmEndPHV.toString());
		 PhaseVector foilPHV = R4BPM01_Foil_phm.times(bpmStartPHV);
		 ...
		 for(int i = 0, n = corrV.size(); i < n; i++) {
			 ...
			 makePhaseMatrix(trajectory);
			 PhaseVector bpmEndPHV_new = R4BPM01_R1BPM01_phm.times(bpmStartPHV);
			 PhaseVector foilPHV_new = R4BPM01_Foil_phm.times(bpmStartPHV);
     ...
     private void makePhaseMatrix(Trajectory<TransferMapState> trajectory){
		 ...
		 //PhaseMatrix phMatrBPM10_End = trajectory.getTransferMatrix(node0,node1);
		 PhaseMatrix phMatrBPM10_End = getTransferMatrix(trajectory, node0, node1);
		 
		 //start - BPM_A13
		 node0 = trajectory.initialState().getElementId();
		 node1 = bpmEnd.getId();		 
		// PhaseMatrix phMatrStart_BPM13 = trajectory.getTransferMatrix(node0,node1);
		 PhaseMatrix phMatrStart_BPM13 = getTransferMatrix(trajectory, node0, node1);
		 
		 R4BPM01_R1BPM01_phm = phMatrStart_BPM13.times(phMatrBPM10_End);
		 //System.out.println("debug BPM10_BPM13_phm="+BPM10_BPM13_phm.toString());
		 
		 //BPM10 - Foil
		 node0 = bpmStart.getId();
		 node1 = foil.getId();		 
		// R4BPM01_Foil_phm = trajectory.getTransferMatrix(node0,node1);
		 R4BPM01_Foil_phm = getTransferMatrix(trajectory, node0, node1);
